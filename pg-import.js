...
const { waitForEnter, createDumpFolder, isValidImport } = require('./helper');

configFile['import'].forEach(el => {
  ...
  const schemaFileDir = dumpFolder + '/schema';
  const dataFileDir = dumpFolder + '/data';
  ...
  if (!onlyDump) {
    ...
    try {
      ...
      waitForEnter(() => {
        ...
        if (removeFiles) {
          fs.unlinkSync(path.join(dumpDirID, 'schema'));
          fs.unlinkSync(path.join(dumpDirID, 'data'));
        }
